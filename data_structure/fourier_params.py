...
def estimateBandwidth(parameters):
    ...
    for dimIdx in ["x", "y"]:
        ...
        if isinstance(sensor_distance_m, (int, float)):
        ...
        elif isinstance(sensor_distance_m, np.ndarray):
            Nf = (ms_length_m / 2.0) ** 2 / wavelength_m / sensor_distance_m[0]

            # Define Fourier bandwidth based on Fresnel Number Regime
            if Nf < 0.25:
                bandwidth = 1 / ms_length_m
            else:
                bandwidth = ms_length_m / wavelength_m / sensor_distance_m[0]
