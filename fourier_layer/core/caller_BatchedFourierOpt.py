...
def loopWavelength_psf_measured(ms_trans, ms_phase, normby, point_source_locs, parameters_list,z):
...
def loopWavelength_field_propagation(field_amplitude, field_phase, parameters_list,z):
    ...
    if input_rank == tf.TensorShape(3):
        ...
        def lambda_loopBody(idx_, holdField_ampl_, holdField_phase_):
            ampl, phase = field_propagation(field_amplitude, field_phase, parameters_list[idx_],z)
        ...
        def lambda_loopBody(idx_, holdField_ampl_, holdField_phase_):
            ampl, phase = field_propagation(field_amplitude[idx_], field_phase[idx_], parameters_list[idx_],z)
            holdField_ampl_ = tf.concat([holdField_ampl_, tf.expand_dims(ampl, 0)], axis=0)
            holdField_phase_ = tf.concat([holdField_phase_, tf.expand_dims(phase, 0)], axis=0)
