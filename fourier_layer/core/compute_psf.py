...
#####################################
def psf_measured(point_source_locs, ms_modulation_trans, ms_modulation_phase, parameters, normby_transmittance,z):
    """Computes the point-spread function at the sensor-plane then resamples and integrates to yield measurement on a
    ...
    # compute the PSF at the sensor plane -- note that psf_sensor returns
    # tf.math.abs(field)**2 already with appropriate psf normalization on energy!
    calc_modulation_intensity, calc_modulation_phase = psf_sensor(point_source_locs, ms_modulation_trans, ms_modulation_phase, parameters, normby_transmittance,z)
...
def psf_sensor(point_source_locs, ms_modulation_trans, ms_modulation_phase, parameters, normby_transmittance,z, convert_2D=True):
    ...
    # Get the field after the metasurface, given a point-source spherical wave origin
    calc_modulation_trans, calc_modulation_phase = wavefront_pointSources_afterms(point_source_locs, calc_modulation_trans, calc_modulation_phase, parameters,z)

    # get finely sampled field just above the sensor
    (calc_modulation_trans, calc_modulation_phase) = wavefront_afterms_sensor(calc_modulation_trans, calc_modulation_phase, parameters,z)
...
def field_propagation(field_amplitude, field_phase, parameters,z):
    """Takes a batch of field amplitudes and field phases at an input plane (of a single wavelength) and propagates the
    ...
    field_amplitude, field_phase = wavefront_afterms_sensor(tf.expand_dims(field_amplitude, 1), tf.expand_dims(field_phase, 1), parameters,z)
