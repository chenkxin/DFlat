...
class PSF_Layer(tf.keras.layers.Layer):
    ...
    def __call__(self, inputs, point_source_locs, batch_loop=False):
        ...
        if batch_loop:
            ...
            out = loopWavelength_psf_measured(ms_trans, ms_phase, self.__sqrt_energy_illum, point_source_locs, self.parameters_list,self.z)
...
class Propagate_Planes_Layer(tf.keras.layers.Layer):
    ...
    def __call__(self, inputs, batch_loop=False):
        ...
        if batch_loop:
            ...
            out = loopWavelength_field_propagation(field_amplitude, field_phase, self.parameters_list,self.z)
